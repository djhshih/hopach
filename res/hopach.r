...
p <- pam(D.sub, 2)
write.table(p$clustering, "pam_c56_k2.txt", row.names=FALSE, col.names=FALSE, sep="\t")
saveRDS(p, "pam_c56_k2.rds")

p <- pam(D.sub, 3)
write.table(p$clustering, "pam_c56_k3.txt", row.names=FALSE, col.names=FALSE, sep="\t")
saveRDS(p, "pam_c56_k3.rds")
